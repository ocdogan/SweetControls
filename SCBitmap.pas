...
uses
  Windows, Messages, SysUtils, Classes, Graphics, SCCommon;

type
  TSCBitmapImage = class(TSharedImage)
  private
    FHandle: HBITMAP;
    FMaskHandle: HBITMAP;
    FPalette: HPALETTE;
  ...
  TSCBitmap = class(TGraphic)
  private
    FImage: TSCBitmapImage;
    FCanvas: TCanvas;
...
{ Create a canvas that gets its DC from the memory DC cache }
  ...
  public
    constructor Create(ABitmap: TSCBitmap);
    destructor Destroy; override;
  end;
...
begin
  ...
  try
    for I := Count - 1 downto 0 do
      with TSCBitmapCanvas(Items[I]) do
...
procedure TSCBitmap.SetBounds(AWidth, AHeight: Integer);
var
  DIB: TDIBSection;
begin
  with FImage do
...
destructor TSCBitmapImage.Destroy;
begin
...
procedure TSCBitmapImage.FreeHandle;
begin
...
constructor TSCBitmapCanvas.Create(ABitmap: TSCBitmap);
begin
  inherited Create;
  FBitmap := ABitmap;
end;

destructor TSCBitmapCanvas.Destroy;
begin
  FreeContext;
  inherited Destroy;
end;

procedure TSCBitmapCanvas.FreeContext;
var
  H: HBITMAP;
begin
  if FHandle <> 0 then
...
procedure TSCBitmapCanvas.CreateHandle;
var
  H: HBITMAP;
begin
  if FBitmap <> nil then
...
constructor TSCBitmap.Create;
begin
  inherited Create;
  FTransparentColor := clDefault;
  FImage := TSCBitmapImage.Create;
  FImage.Reference;
  if DDBsOnly then HandleType := bmDDB;
end;

destructor TSCBitmap.Destroy;
begin
  FreeContext;
  FImage.Release;
  FCanvas.Free;
...
procedure TSCBitmap.Assign(Source: TPersistent);
var
  DIB: TDIBSection;
begin
  if (Source = nil) or (Source is TSCBitmap) then
  begin
    EnterCriticalSection(BitmapImageLock);
    try
      if Source <> nil then
      begin
        TSCBitmap(Source).FImage.Reference;
        FImage.Release;
        FImage := TSCBitmap(Source).FImage;
        Transparent := TSCBitmap(Source).Transparent;
        FTransparentColor := TSCBitmap(Source).FTransparentColor;
        FTransparentMode := TSCBitmap(Source).FTransparentMode;
      end
      else
      begin
...
procedure TSCBitmap.CopyImage(AHandle: HBITMAP; APalette: HPALETTE; DIB: TDIBSection);
var
...
{ Called by the FCanvas whenever an operation is going to be performed on the
  ...
  TSCBitmap, the handle needs to be 'cloned' if it is being refered to by more
  than one TSCBitmap }
procedure TSCBitmap.Changing(Sender: TObject);
begin
  FreeImage;
...
procedure TSCBitmap.Changed(Sender: TObject);
begin
  FMaskBitsValid := False;
  inherited Changed(Sender);
end;

procedure TSCBitmap.Dormant;
begin
...
procedure TSCBitmap.Draw(ACanvas: TCanvas; const Rect: TRect);
var
  OldPalette: HPalette;
...
begin
  with Rect, FImage do
  begin
    TSCFakeCanvas(ACanvas).RequiredState(csAllValid);
    PaletteNeeded;
    OldPalette := 0;
    ...
    try
      ...
      TSCFakeCanvas(Canvas).RequiredState(csAllValid);
      if Transparent then
      begin
        Save := 0;
        MaskDC := 0;
...
function TSCBitmap.GetEmpty;
begin
  with FImage do
    Result := (FHandle = 0) and (FDIBHandle = 0);
end;

function TSCBitmap.GetCanvas: TCanvas;
begin
  if FCanvas = nil then
  begin
    HandleNeeded;
    FCanvas := TSCBitmapCanvas.Create(Self);
...
{ Since the user might modify the contents of the HBITMAP it must not be
  shared by another TSCBitmap when given to the user nor should it be selected
  into a DC. }
function TSCBitmap.GetHandle: HBITMAP;
begin
  FreeContext;
  HandleNeeded;
  Changing(Self);
...
function TSCBitmap.GetHandleType: TBitmapHandleType;
begin
  with FImage do
  begin
...
function TSCBitmap.GetHeight: Integer;
begin
  Result := Abs(FImage.FDIB.dsbm.bmHeight);
end;

function TSCBitmap.GetMaskHandle: HBITMAP;
begin
  MaskHandleNeeded;
  Result := FImage.FMaskHandle;
end;

function TSCBitmap.GetMonochrome: Boolean;
begin
  with FImage.FDIB.dsbm do
    Result := (bmPlanes = 1) and (bmBitsPixel = 1);
end;

function TSCBitmap.GetPalette: HPALETTE;
begin
  PaletteNeeded;
  Result := FImage.FPalette;
end;

function TSCBitmap.GetPixelFormat: TPixelFormat;
begin
  Result := pfCustom;
...
function TSCBitmap.GetScanLine(Row: Integer): Pointer;
begin
  Changing(Self);
...
function TSCBitmap.GetTransparentColor: TColor;
begin
...
function TSCBitmap.GetWidth: Integer;
begin
  Result := FImage.FDIB.dsbm.bmWidth;
end;

procedure TSCBitmap.DIBNeeded;
begin
  with FImage do
  begin
...
procedure TSCBitmap.FreeContext;
begin
  if (FCanvas <> nil) then TSCBitmapCanvas(FCanvas).FreeContext;
end;

procedure TSCBitmap.HandleNeeded;
begin
  with FImage do
    if FHandle = 0 then
      FHandle := FDIBHandle;
...
procedure TSCBitmap.Mask(TransparentColor: TColor);
var
  NewHandle, NewPalette: THandle;
  DIB: TDIBSection;
begin
...
procedure TSCBitmap.MaskHandleNeeded;
begin
...
procedure TSCBitmap.PaletteNeeded;
var
  DC: HDC;
begin
  with FImage do
...
procedure TSCBitmap.LoadFromClipboardFormat(AFormat: Word; AData: THandle;
  APalette: HPALETTE);
var
  DIB: TDIBSection;
begin
...
procedure TSCBitmap.LoadFromStream(Stream: TStream);
begin
  ReadStream(Stream, Stream.Size - Stream.Position);
end;

procedure TSCBitmap.LoadFromResourceName(Instance: THandle; const ResName: string);
var
...
procedure TSCBitmap.LoadFromResourceID(Instance: THandle; ResID: Integer);
var
  Stream: TCustomMemoryStream;
begin
...
procedure TSCBitmap.NewImage(NewHandle: HBITMAP; NewPalette: HPALETTE;
  const NewDIB: TDIBSection; OS2Format: Boolean; RLEStream: TStream = nil);
var
  Image: TSCBitmapImage;
begin
  Image := TSCBitmapImage.Create;
  with Image do
  try
    FHandle := NewHandle;
    FPalette := NewPalette;
...
procedure TSCBitmap.ReadData(Stream: TStream);
var
  Size: Longint;
begin
...
procedure TSCBitmap.ReadDIB(Stream: TStream; ImageSize: LongWord);
const
        ...
        begin
          ...
          try
            ...
            try
              ...
              if (BMHandle = 0) then
                if GetLastError = 0 then InvalidBitmap else scRaiseLastOSError;
            finally
              if OldPal <> 0 then
...
procedure TSCBitmap.ReadStream(Stream: TStream; Size: Longint);
var
...
procedure TSCBitmap.SetHandle(Value: HBITMAP);
var
  DIB: TDIBSection;
  APalette: HPALETTE;
begin
...
procedure TSCBitmap.SetHandleType(Value: TBitmapHandleType);
var
  DIB: TDIBSection;
  AHandle: HBITMAP;
  NewPalette: HPALETTE;
...
procedure TSCBitmap.SetHeight(Value: Integer);
begin
  Self.SetBounds(FImage.FDIB.dsbm.bmWidth, Value);
end;

procedure TSCBitmap.SetMaskHandle(Value: HBITMAP);
begin
  with FImage do
...
procedure TSCBitmap.SetMonochrome(Value: Boolean);
var
  DIB: TDIBSection;
begin
  with FImage, FDIB.dsbmih do
...
procedure TSCBitmap.SetPalette(Value: HPALETTE);
var
  AHandle: HBITMAP;
  DIB: TDIBSection;
begin
...
procedure TSCBitmap.SetPixelFormat(Value: TPixelFormat);
const
...
procedure TSCBitmap.SetTransparentColor(Value: TColor);
begin
...
procedure TSCBitmap.SetTransparentMode(Value: TTransparentMode);
begin
...
procedure TSCBitmap.SetWidth(Value: Integer);
begin
  Self.SetBounds(Value, FImage.FDIB.dsbm.bmHeight);
end;

procedure TSCBitmap.WriteData(Stream: TStream);
begin
  WriteStream(Stream, True);
end;

procedure TSCBitmap.WriteStream(Stream: TStream; WriteSize: Boolean);
const
...
begin
  ...
  begin
    ...
    begin
      ...
      TSCFakeCanvas(Canvas).RequiredState([csHandleValid]);
...
{ ReleaseHandle gives up ownership of the bitmap handle the TSCBitmap contains. }
function TSCBitmap.ReleaseHandle: HBITMAP;
begin
  HandleNeeded;
  Changing(Self);
  with FImage do
...
function TSCBitmap.ReleaseMaskHandle: HBITMAP;
begin
  Result := GetMaskHandle;
  FImage.FMaskHandle := 0;
end;

function TSCBitmap.ReleasePalette: HPALETTE;
begin
  HandleNeeded;
  Changing(Self);
  Result := FImage.FPalette;
...
procedure TSCBitmap.SaveToStream(Stream: TStream);
begin
  WriteStream(Stream, False);
end;

procedure TSCBitmap.SaveToClipboardFormat(var Format: Word; var Data: THandle;
...
function TSCBitmap.TransparentColorStored: Boolean;
begin
